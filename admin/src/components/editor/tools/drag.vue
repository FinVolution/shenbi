<template>
  <div class="drap-resize_main">
    ...
    <!-- <div class="drap-rotate" @mousemove.stop="move" @mousedown="handleRotateDown">
...
export default {
  ...
  props: ['elWidth', 'elHeight', 'cursorTop', 'cursorLeft','componentId'],
  watch: {
    elWidth(val) {
      this.width = val
    },
  ...
  methods: {
    ...
    handleRotateDown(e) {
      const dom = document.getElementById(`id_component_${this.componentId}`)
      ...
      this.rotating = true;
      console.log('handleRotateDown',top, left, width, height)
    },
    ...
    move(ev) {
      ...
      this.rotate = (this.getAngle(x, y) - 90) % 360 || 0;
      console.log('rotate',this.rotate)
      this.$emit('onDragRotate', this.rotate);
      this.changeComponentAttr({ key: 'selfStyle', newConfig: { rotate: `${this.rotate}deg` } })
