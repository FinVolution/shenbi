<template>
  <div class="choose-container" @click="onMainContainerClick">
    ...
    <ul class="choose-list" v-if="componentList && componentList.length > 0">
      <div :class="{ 'thumbnail-home-item': true, 'is-active': true }" style="zoom: 35%;" @click="changeToolTabType('page-config')">
        <HomePage style="pointer-events: none;"/>
      </div>
      <draggable v-model="componentList" group="cplistForChose" @start="drag = true" @end="drag = false">
        <transition-group>
          <li v-for="item in componentList" :key="`choose_item_${item.id}`" @click="onCompontItemSelect(item)"
            @contextmenu.prevent="rightClick($event, item, 'outer')">
            <span class="desc"> {{ item.name }} </span>
            <div :class="{ 'thumbnail-item': true, 'is-active': item.isActive }">
              <div class="content" :class="{ 'has-border-bottom': item.children && item.children.length > 0 }">
                <div style="zoom: 35%;">
                  <CompList :item="item" :activeComponent="activeComponent" />
                </div>
              </div>
              <ul class="child-list" v-if="item.children && item.children.length > 0">
                <li class="child-item content"
                  :class="{ active: activeComponent && activeComponent.id === childItem.id }"
                  v-for="childItem in item.children" :key="`child_item_${childItem.id}`"
...
export default {
  ...
  components: { draggable, CompList, HomePage },
  computed: {
    ...mapState('editor', {
  ...
  methods: {
    ...mapMutations('editor', ['changeComponentAcitve', 'changeRightClick', 'changeToolTabType']),
    // 组件右键菜单
...
.choose-container {
  ...
  .thumbnail-home-item {
    ...
    /deep/ .palette-container .editor-container {
