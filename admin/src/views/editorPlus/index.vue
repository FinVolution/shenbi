...
import DesignEditor from '~/editor/mainPlus/index'
import ToolIndex from '~/editor/toolsPlus'
import Chose from '~/editor/mainPlus/chose'
import Panel from '~/editor/mainPlus/panel'
...
export default {
  ...
  methods: {
    ...mapMutations('tenant',['setCurrentTenantIdById']),
  ...
  created() {
    //全局获取项目id和草稿id
    let { projectId, preId, type, tenantId } = this.$route.query || {}
    this.setObject({ key: 'projectId', val: projectId })
    this.setObject({ key: 'preId', val: preId })
    this.setCurrentTenantIdById(tenantId)
