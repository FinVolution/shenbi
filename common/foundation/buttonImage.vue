<template>
    <img :src="forceHttps(property.url || defaultImg)" class="editor-component-img" :style="animStyle"
    :class="[{ 'image-fit': !property.url , 'not-app-marign' : !isApp && property.setAppMarignTop, anim}]"
    @click="onImageClick" />
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    animStyle(){
      ...
      const style = generateStyleByObj(this.selfStyle)
      console.log(style)
      return {
        ...style,
        'animation-iteration-count':loop,
        'animation-duration':this.property.animDuration,
      }
    }
  },
  watch: {
